...
export function ToothPanel({
  ...
  numero: number
  record?: ToothRecord
  onSave: (patch: { status: ToothStatus; observacao: string }) => void
  onClose: () => void
}) {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Dente</p>
      ...
      <div className="mt-4 flex gap-2">
        ...
        <Button className="flex-1" onClick={() => onSave({ status, observacao: observacao.trim() })}>
          Salvar
        </Button>
      </div>
    </div>
